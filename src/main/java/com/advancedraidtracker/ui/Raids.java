...
public class Raids extends BaseFrame implements UpdateableWindow
{
	...
	public void createFrame()
	{
		...
		JMenuItem openRaidInExplorer = getThemedMenuItem("Open Raid In File Explorer");
		openRaidInExplorer.addActionListener(e ->
		{
	...
	public static void openFileLocation(Path path)
	{
		...
		try
		{
			if (Desktop.isDesktopSupported())
			{
				...
				// Check if the BROWSE_FILE_DIR action is supported (Java 9+)
				...
				else
				{
					...
					openFileLocationOSSpecific(file);
				}
			}
			else
			{
				// Desktop API is not supported, use OS-specific commands
				openFileLocationOSSpecific(file);
			}
		}
		catch (IOException e)
	...
	private static void openFileLocationOSSpecific(File file) throws IOException
	{
		...
		{
			// Mac OS: Use "open" with "-R" to reveal the file in Finder
			new ProcessBuilder("open", "-R", absolutePath).start();
		}
		else
		{
			// Linux and other Unixes
			// Try xdg-open which opens the default file manager
			...
			{
				// If parent directory is null, open the file itself
